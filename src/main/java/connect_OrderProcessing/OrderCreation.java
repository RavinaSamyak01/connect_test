...
import connect_OrderCreationNonSPL.LOC;
import connect_OrderCreationNonSPL.AIR;
import connect_OrderCreationNonSPL.DRV;
import connect_OrderCreationNonSPL.FRA;
import connect_OrderCreationNonSPL.FRG;
import connect_OrderCreationNonSPL.P3P;
import connect_OrderCreationNonSPL.PA;
import connect_OrderCreationNonSPL.SD;
import connect_OrderCreationNonSPL.SDC;
import connect_OrderCreationSPL.CPU;
import connect_OrderCreationSPL.D3P;
import connect_OrderCreationSPL.H3P;
import connect_OrderCreationSPL.T3PLAST;
...
public class OrderCreation extends BaseInit {
	...
	public void allOrderCreation() throws Exception {
		...
		try {
			 ...
			 * "PASS");
			 * 
			 * } catch (Exception SD) { logs.info(SD); getScreenshot(driver, "SDIssue");
			 ...
			 * 4, "PASS");
			 * 
			 * } catch (Exception PA) { logs.info(PA); getScreenshot(driver, "PAIssue");
			 * String Error = PA.getMessage(); setResultData("Result", 4, 4, "FAIL");
			 * setResultData("Result", 4, 5, Error); login();
			 * 
			 * }
			 */
			try {
			 ...
			 * 4, "PASS");
			 * 
			 * } catch (Exception AIR) { logs.info(AIR); getScreenshot(driver, "AIRIssue");
			 * String Error = AIR.getMessage(); setResultData("Result", 6, 4, "FAIL");
			 * setResultData("Result", 6, 5, Error); login();
			 * 
			 * }
			 */
			 ...
			 * "PASS");
			 * 
			 * } catch (Exception H3P) { logs.info(H3P); getScreenshot(driver, "LOCIssue");
			 * String Error = H3P.getMessage(); setResultData("Result", 10, 4, "FAIL");
			 * setResultData("Result", 10, 5, Error); login();
			 * 
			 * }
			 */
			 ...
			 * 4, "PASS");
			 * 
			 * } catch (Exception CPU) { logs.info(CPU); getScreenshot(driver, "CPUIssue");
			 * String Error = CPU.getMessage(); setResultData("Result", 13, 4, "FAIL");
			 * setResultData("Result", 13, 5, Error); login();
			 * 
			 * }
			 */
			...
			System.out.println("Issue in Order Creation from connect");
			logs.info("Issue in Order Creation from connect");
			// --Connect LogOut
			Env = storage.getProperty("Env");
			if (Env.equalsIgnoreCase("Test")) {
		...
		String subject = "Selenium Automation Script: " + Env + " Connect Order Creation";
		try {
